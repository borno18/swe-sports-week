...
export default async function Home() {
  ...
  return <>
    <section className="hero">
      ...
      <div className="hero-content"><div className="live-kicker">The official tournament hub</div><h1><span>INTRA SWE</span>SPORTS WEEK</h1><p className="hero-year">{event.year}</p><div className="hero-details"><span><CalendarDays size={18} />{event.dates}</span><span><MapPin size={18} />SUST Campus</span></div><p className="hero-copy">One campus. Every batch. Follow every player, every match, and every path to the final.</p><div className="hero-actions"><Link href="#tournaments" className="button primary">Explore the brackets <ArrowRight size={18} /></Link><Link href="/schedule" className="button ghost">View match schedule</Link></div><EventCountdown /></div>
    ...
    {recent.length > 0 && <section className="section"><div className="section-heading"><div><span className="eyebrow">Latest outcomes</span><h2>Through to the next round</h2></div><Link href="/results">All results <ChevronRight size={17} /></Link></div><div className="match-grid">{recent.map(match => <MatchCard key={match.id} match={match} />)}</div></section>}
